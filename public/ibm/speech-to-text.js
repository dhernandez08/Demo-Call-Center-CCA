  ...
  function micON() { // When the microphone button is clicked
    if (recording === false ) {
      ...
      //  Api.setWatsonPayload({output: {text: ['Accept the microphone prompt in your browser. Watson will listen soon.'], ref: 'STT'}}); // Dialog box output to let the user know we're recording
        ...
        closeCall.setAttribute('class', 'inactive-close'); 
        records++;
        button.value = 'ON';
        ...
        stream.promise()                                // Once all data has been processed...
          .then(function(data) {                        // ...put all of it into a single array
            ...
            if (data.length !== 0) {                    // If data is not empty (the user said something)
              ...
              Api.setWatsonPayload({output: {text: ['Entrada de microfono  desconectada, por favor llame otra vez']}}); // If the user clicked the microphone button again to cancel current input
            }
          })
          .catch(function(err) { // Catch any errors made during the promise
            if (err !== 'Error: No se identifica mensaje en 5s') { // This error will always occur when Speech-To-Text times out, so don't log it (but log everything else)
              console.log(err);
            } 
            else{
           // mic.setAttribute('class', 'inactive-mic'); // Reset our microphone button to visually indicate we aren't listening to user anymore
           if(fin === 0){
             Api.setWatsonPayload({output: {text: ['Se ha terminado la llamada después de no evidenciar actividad.']}});

             Api.setWatsonPayload({output: {ref: 'STT'}}); 
             console.log("primera vez se termina");
             fin++;
           }
           else{
             Api.setWatsonPayload({output: {ref: 'STT'}}); 
             console.log("segunda y mas veces se termina");
           }
            Api.setWatsonPayload({output: {ref: 'STT'}}); 
